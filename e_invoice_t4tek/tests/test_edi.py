...
class TestVNEDI(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls):
        ...
        cls.template = cls.env['e_invoice_t4tek.e_invoice.template'].create({
            'name': '1/001',
            'template_invoice_type': '1',
        })
        cls.symbol = cls.env['e_invoice_t4tek.e_invoice.symbol'].create({
            'name': 'K24TUT',
        ...
        cls.env.company.write({
            'street': '287/54 Âu Dương Lân, Phường Rạch Ông, Quận 8, Hồ Chí Minh',
            'state_id': cls.env.ref('base.state_vn_VN-HN').id,
            'country_id': cls.env.ref('base.vn').id,
            'vat': '00000000000',
            'phone': '0965 643 046',
            'email': 'test_company@t4tek.com',
    ...
    def test_default_symbol_on_partner(self):
        ...
        self.partner_a.l10n_vn_edi_symbol = self.env['e_invoice_t4tek.e_invoice.symbol'].create({
            'name': 'K24TUD',
    ...
    def test_send_and_print(self):
        ...
        self.assertNotEqual(invoice.l10n_vn_edi_e_invoice_xml_file, False)
        self.assertNotEqual(invoice.l10n_vn_edi_e_invoice_pdf_file, False)
        self.assertNotEqual(invoice.l10n_vn_edi_e_invoice_file, False)
    ...
    def test_cancel_invoice(self):
        ...
        self.assertEqual(action['res_model'], 'e_invoice_t4tek.cancellation')
        with patch('odoo.addons.e_invoice_t4tek.models.account_move._l10n_vn_edi_send_request', return_value=(None, None)):
            self.env['e_invoice_t4tek.cancellation'].create({
    ...
    def test_access_token(self):
        ...
        with patch('odoo.addons.e_invoice_t4tek.models.account_move._l10n_vn_edi_send_request', return_value=(request_response, None)):
    ...
    def _send_invoice(self, invoice):
        ...
        with patch('odoo.addons.e_invoice_t4tek.models.account_move.AccountMove._l10n_vn_edi_fetch_invoice_pdf_file_data', return_value=pdf_response), \
             patch('odoo.addons.e_invoice_t4tek.models.account_move.AccountMove._l10n_vn_edi_fetch_invoice_xml_file_data', return_value=xml_response), \
             patch('odoo.addons.e_invoice_t4tek.models.account_move._l10n_vn_edi_send_request', return_value=(request_response, None)):
