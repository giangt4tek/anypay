...
# Invoice template that needs to be passed to e_invoice and will determine the format of the resulting
# invoice pdf on their system
class e_invoiceTemplate(models.Model):
    _name = 'e_invoice_t4tek.e_invoice.template'
    _description = 'e_invoice template'
    ...
    template_invoice_type = fields.Selection(
        selection=[
            ...
            ('HDGTGT', 'HÓA ĐƠN GIÁ TRỊ GIA TĂNG'),
            ('HDGTGTMTT', 'HÓA ĐƠN GIÁ TRỊ GIA TĂNG MÁY TÍNH TIỀN'),
            ('HDBH', 'HÓA ĐƠN BÁN HÀNG'),
            ('HDBHMTT', 'HÓA ĐƠN BÁN HÀNG MÁY TÍNH TIỀN'),
        ],
        required=True,
    )
    ...
    def _constrains_changes(self):
        ...
        for any invoices sent to e_invoice.
        """
...
# Invoice symbol that needs to be passed to e_invoice and will determine the prefix of the
# invoice number on their system
class e_invoiceSymbol(models.Model):
    _name = 'e_invoice_t4tek.e_invoice.symbol'
    _description = 'e_invoice symbol'
    """
    ...
    invoice_template_id = fields.Many2one(
        comodel_name='e_invoice_t4tek.e_invoice.template',
        required=True,
    )
