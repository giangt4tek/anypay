...
class AccountMoveSend(models.AbstractModel):
    ...
    def _get_all_extra_edis(self) -> dict:
        ...
        res.update({'vn_e_invoice_send': {'label': _("E-Invoice Not Sign "), 'is_applicable': self._is_vn_edi_applicable}})
    ...
    def _get_invoice_extra_attachments(self, move):
        ...
        return (
            ...
            + move.l10n_vn_edi_e_invoice_xml_file_id
    ...
    @api.model
    def _generate_e_invoice_file_date(self, invoice, invoice_data):
        # Ensure that we still generate the file if 'generate' is ul10n_vn_edi_invoice_transaction_id-checked but send it.
        
        need_file = (
            (invoice_data['invoice_edi_format'] == 'vn_e_invoice' and invoice._l10n_vn_edi_get_credentials_company())
            or 'vn_e_invoice_send' in invoice_data['extra_edis']
        )
      
        # In case we already have a json file existing on the invoice, we skip regenerating it.
        if need_file and not invoice.l10n_vn_edi_e_invoice_file:
            ...
            if not errors:
                ...
                invoice_data['e_invoice_attachments'] = [{
                    ...
                    'res_id': invoice.id,
                    'res_field': 'l10n_vn_edi_e_invoice_file',  # Binary field
                }]
            else:
                invoice_data['error'] = {
                    'error_title': _('Error when generating e_invoice file.'),
                    'errors': errors,
                }
    ...
    def _hook_invoice_document_before_pdf_report_render(self, invoice, invoice_data):
        ...
        self._generate_e_invoice_file_date(invoice, invoice_data)
    ...
    def _call_web_service_before_invoice_pdf_render(self, invoices_data):
        ...
        for invoice, invoice_data in invoices_data.items():
            ...
            if 'vn_e_invoice_send' in invoice_data['extra_edis']:
                errors = invoice._l10n_vn_edi_check_invoice_configuration()
                
                if not errors:
                    if 'e_invoice_attachments' in invoice_data:
                        json_data = json.loads(invoice_data['e_invoice_attachments'][0]['raw'].decode('utf-8'))
                    # If the invoice was downloaded but not sent, the json file could already be there.
                    elif invoice.l10n_vn_edi_e_invoice_file:
                        json_data = json.loads(base64.b64decode(invoice.l10n_vn_edi_e_invoice_file).decode('utf-8'))
                    # If we don't have the file data and the file, we will regenerate it.
                        ...
                        if 'e_invoice_attachments' not in invoice_data:
                            continue
                        json_data = json.loads(invoice_data['e_invoice_attachments'][0]['raw'].decode('utf-8'))
                    if json_data:
                ...
                if errors:
                    invoice_data['error'] = {
                        'error_title': _('Error when sending to e_invoice'),
                        'errors': errors,
                    }
    ...
    # -------------------------------------------------------------------------
    # Không gọi file PDF của hóa đơn điện tử
    # Vì gây gây ra nặng dữ liệu, và không cần thiết khi lưu trữ hóa đơn điện tử
    # -------------------------------------------------------------------------
    ...
    #         # if json_pdf_data:
    #         #         raw_content = json_pdf_data.pop("raw", None)
    #         #         if raw_content:
    #         #            json_pdf_data["datas"] = base64.b64encode(raw_content).decode("utf-8")
    #         #            json_pdf_data["type"] = "binary"
    #                    #_logger.info("-------> file_pdf_data = %s", json_pdf_data)

            
    #         #_logger.info("-------------->json_filePDF_data = %s", json_filePDF_data)
    #         if not invoice.l10n_vn_edi_e_invoice_file and json_file_data:
    #             self.env['ir.attachment'].with_user(SUPERUSER_ID).create(json_file_data)
    #             invoice.invalidate_recordset(fnames=[
    #                 'l10n_vn_edi_e_invoice_file_id',
    #                 'l10n_vn_edi_e_invoice_file',
    #             ])
            
    #         if invoice.l10n_vn_edi_invoice_state != 'sent':
    #             continue
            
    #         # Download e_invoice documents in order to attach them to the email we sent to the customer.
    ...
    #             invoice_data['error'] = {
    #                 'error_title': _('Error when receiving e_invoice files.'),
    ...
    #             invoice.invalidate_recordset(fnames=[
    #                 'l10n_vn_edi_e_invoice_xml_file_id',
    #                 'l10n_vn_edi_e_invoice_xml_file',
    #                 'l10n_vn_edi_e_invoice_pdf_file_id',
    #                 'l10n_vn_edi_e_invoice_pdf_file',
    #             ])
                
    #             # Log the new attachment in the chatter for reference. Make sure to add the JSON file.
    #             invoice.with_context(no_new_invoice=True).message_post(
    #                 body=_('Invoice sent to e_invoice'),
    #                 attachment_ids=attachments.ids + invoice.l10n_vn_edi_e_invoice_file_id.ids,
    #             )
            
    #         self._get_invoice_extra_attachments(invoice)
    #         if self._can_commit():
    #             self._cr.commit()
