...
class T4tekWalletAccount(models.Model):
    _name = 't4tek.wallet.account'
    _description = 'Quản lý API cho từng tài khoản Ví AnyPay'

    #cccd_ids = fields.Many2one('res.partner', string="Căn cước công dân", required=True)
    #wallet_id = fields.Many2one('res.partner.wallet', string="Tài khoản Ví AnyPay", required=True)
    ...
    def create(self, vals_list):
        ...
        if partner_ids:
            ...
            if existing:
                raise UserError("Một số người dùng đã có tài khoản Ví AnyPay.")
    ...
    _sql_constraints = [
    ('partner_id_unique', 'unique(partner_id)', 'Người dùng này đã có tài khoản Ví AnyPay.')]
