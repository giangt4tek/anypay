...
export class TransactionDashboard extends Component {
  ...
  // Hàm khởi tạo, lấy dữ liệu người dùng và tài khoản Ví AnyPay
  ...
  async initialize() {
    ...
    data = await this.orm.searchRead(
      "t4tek.wallet.account",
      [["partner_id", "=", this.state.resId]],
      ["acc_number", "balance_account"]
    );
    if (data.length === 0) {
      this.notification.add(
        _t("Bạn chưa có tài khoản Ví AnyPay. Vui lòng liên hệ quản trị viên."),
        { type: "danger" }
      );
...
////-----------end select tab---------------///////////// 
  ...
  openDepositDialog() {
    let action = {
      type: "ir.actions.client",
      tag: "anypay_wallet.action_transaction",
      target: "new",
      name: "NẠP TIỀN",
      params: {
...
registry
  .category("actions")
  .add("anypay_wallet.transaction_dashboard", TransactionDashboard);
