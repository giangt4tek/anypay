...
class ApiKey(models.Model):
    ...
    token = fields.Char(
        ...
        required=True,
        readonly=True,
        default=lambda self: hashlib.md5(str(uuid.uuid4()).encode()).hexdigest()
    )
    ...
    _sql_constraints = [
        ('unique_key', 'unique(token)', 'API key phải là duy nhất.')
    ]
    ...
    def is_valid(self, key, ip=None, scope_required=None):
        record = self.search([('token', '=', key), ('active', '=', True)], limit=1)
        if not record:
