...
class ResPartner(models.Model):
    ...
    #     if user.has_group('bank_server.user_bank'):
    #         action = {
    ...
    def _action_dashboard(self):
        user = self.env.user
        if user.has_group('bank_server.manager_bank'):
           ...
           view_form_id = self.env.ref(
               'bank_server.form_view_res_partner').id
           action = {
           ...
           return action
        elif user.has_group('bank_server.user_bank'):
            action = {
                'type': 'ir.actions.client',
                'tag': 'bank_server.transaction_dashboard',
            }
    ...
    def _action_dashboard_manager(self):
        ...
        if self.env.user.has_group('bank_server.manager_bank'):
            context = {'create': False, 'edit': True, 'delete': False},
    
        view_kanban_id = self.env.ref('base.res_partner_kanban_view').id
        view_list_id = self.env.ref('base.view_partner_list').id
        view_form_id = self.env.ref(
            'bank_server.form_view_res_partner_manager').id
        action = {
            'type': 'ir.actions.act_window',
            'res_model': 'res.partner',
        }
