...
export class ActionTransation extends Component {
  setup() {
    this.state = useState({
      ...
      bankList: this.props.action.params.bankList,
      transferBank:''
    });
  ...
  onBankChange(ev) {
    this.state.transferBank = ev.target.value;
  ...
  async onSubmit(ev) {
    ...
    try {
      ...
      if (this.state.transactionType === 'transfer_out')
      {
         ...
         if (this.state.transferBank && this.state.transferBank.trim() == '') 
         {
              this.showNotification(
...
ActionTransation.template = "bank_server.action_transaction";
ActionTransation.props = { ...standardFieldProps };
registry
  .category("actions")
  .add("bank_server.action_transaction", ActionTransation);
